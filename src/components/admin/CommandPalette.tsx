  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed left-1/2 top-24 z-50 w-[90vw] max-w-xl -translate-x-1/2 overflow-hidden rounded-2xl border border-[var(--brand-tint-200)] bg-white shadow-2xl focus:outline-none"
          aria-label="Busca rápida"
        >
          <div className="flex items-center gap-2 border-b border-[var(--brand-tint-100)] bg-white px-3 py-2">
            ...
            {results.map((c) => (
              <li key={c.href}>
                <button
                  ...
                  className="flex w-full min-h-[40px] items-center justify-between gap-3 rounded-xl px-3 py-2 text-left hover:bg-[var(--brand-tint-50)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-ring)] focus-visible:ring-offset-2"
                >
                  <span>{c.label}</span>
