...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id:string; versionId:string }> }){
  ...
  const { data: ver, error } = await sb.from('blog_post_versions').select('id,post_id,snapshot,reason,created_at').eq('id', (await ctx.params).versionId).eq('post_id', (await ctx.params).id).maybeSingle();
  if(error) return NextResponse.json({ error: error.message }, { status:500 });
  if(!ver) return NextResponse.json({ error:'not-found' }, { status:404 });
  if(!wantDiff) return NextResponse.json({ version: ver });
  const { data: current, error: curErr } = await sb.from('blog_posts').select('*').eq('id', (await ctx.params).id).maybeSingle();
